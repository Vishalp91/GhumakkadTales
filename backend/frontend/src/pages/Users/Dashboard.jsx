...
export default function Dashboard() {
    ...
    useEffect(() => {
        setTimeout(async () => {
            //Grab userPosts from database
            const {userPosts, email} = await getUserPosts();
            //Update user state
            setUser({...user, email, posts : userPosts});
        }, 100)
    }, []);
    ...
    return (
        <section className="p-4 bg-red-200 mt-10">
            {success && <Success msg = {success}/>}
            {error && <Alert  msg = {error}/>}
            <h1 className="text-3xl text-center text-red-600 mb-10">Dashboard</h1>
                ...
                <div key = {post._id}>
                    <Post post = {post}>
                        <div className="flex justify-evenly items-center">
                            ...
                            <Link to = "/update" className="inline-flex text-orange-700 hover:bg-red-200 p-1 rounded-md" state = {post}>
                            <svg
                            ...
                            <button onClick={() => handleDelete(post._id)} className="inline-flex text-black hover:bg-gray-300 p-1 rounded-md">
                            <svg
                                fill="black"
                                width="24"
                                height="24"
            ...
            <div className="mb-10 w-full flex items-center justify-center">
                        <Link
                            className="inline-flex text-white items-center mt-4 px-6 py-3 shadow-lg shadow-pink-600/50 font-medium bg-orange-700 rounded-lg mb-2"
                            to="/create"
                        >
